  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Kelola informasi data profil pribadi Anda di perpustakaan</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
        <form onSubmit={handleSave} className="p-6 md:p-8">
          ...
          <div className="flex flex-col items-center justify-center pb-8 border-b border-gray-100 mb-8">
             ...
             >
               {formData.avatar_url ? (
                 <img src={formData.avatar_url} alt="Profile" className="h-full w-full object-cover group-hover:opacity-75 transition-opacity" />
               ) : (
             ...
             <h3 className="mt-4 text-xl font-bold text-gray-800">{formData.name || 'Siswa'}</h3>
             <p className="text-sm font-medium text-blue-600 bg-blue-50 px-3 py-1 rounded-full mt-1 border border-blue-100">Anggota Perpustakaan</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Password Text (Read Only Demo)</label>
              <input type="text" name="password_text" required className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-amber-50 text-amber-700 text-[0.95rem]" value={formData.password_text} onChange={handleChange} />
            </div>
          </div>
